...
//          }
			...
			if (typeof planes !== "undefined" && planes.features[1] && planes.features[1].geometry) {
				var dist_bearing = meters_dist_bearing(lonLat.lon, lonLat.lat, planes.features[1].geometry.x, planes.features[1].geometry.y);

				// final result
				var MilesPerNMile = 1.15078;
        		var newHtml = Math.round(dist_bearing.dist / MilesPerNMile) + " nm bearing " + ("00" + Math.round((dist_bearing.bearing + 180) % 360)).slice(-3) + "&deg;";
			} else {
				return;
			}
