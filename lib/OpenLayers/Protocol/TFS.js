 ...
 *  - <OpenLayers.Protocol>
 */
OpenLayers.Protocol.TFS = OpenLayers.Class(OpenLayers.Protocol, {
    ...
    read: function(options) {
		...
		options.params.z = options.tiles[0].z; 
		options.params.x = options.tiles[0].x; 
		options.params.y = options.tiles[0].y; 
        if(options.filter) {
		...
		resp.tileId = options.tiles[0].id;
        if(readWithPOST) {
    ...
    readTile: function(tile, options) {
        ...
        var url = options.url + "?z="+tile.z + ";x="+tile.x + ";y="+tile.y;
        var paramsString = OpenLayers.Util.getParameterString(options.params);
		url = OpenLayers.Util.urlAppend(url, paramsString);
    ...
    readTiles: function(tiles, options) {
        ...
        for(var i=0; i<tiles.length; i++) {
            resp.push(this.readTile(
                tiles[i], OpenLayers.Util.applyDefaults(
                    {callback: callback, scope: this, tileId : tiles[i].id }, options
                ))
            );
        }
        return resp;
