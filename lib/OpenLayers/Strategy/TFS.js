...
OpenLayers.Strategy.TFS = OpenLayers.Class(OpenLayers.Strategy, {
    ...
    /**
     * Porperty: zoomCache
     * {Object} Dictionary of zoom level to the cache zoom mapping. In
     * {'3': 3,'4': 3, '5': 3, '6':6} from zoom 3 to 5 will request tiles of 
     * zoom 3 that intersect the map view.
     */
    zoomCache: null,  
    /**
     * Property: response
    ...
    initialize: function(options) {
		...
		this.mapBounds = options.mapBounds ? options.mapBounds : map.maxExtent;
	...
	//	console.log("Strategy_TFS.update");
        ...
        if ( this.isBoundInvalid(mapBounds)) {
            this.calculateBounds(mapBounds);
            this.resolution = this.layer.map.getResolution(); 
			this.zoom = this.layer.map.getZoom();
      if(!this.lastReqZoom) this.lastReqZoom = this.zoom;
			if (options){
...
//          );
     ...
     * {Boolean} 
     */
    isBoundInvalid: function(mapBounds) {
        if(!mapBounds) {
	...
	//	console.log("reading all tiles upto zoom: " + this.zoom);
		if (this.zoomChanged) {
      if(this.zoomCache[this.zoom+''] != this.zoomCache[this.lastReqZoom+'']) {
			//console.log("removing the old features");
			  this.layer.destroyFeatures();
			  this.visitedTiles = {};
        this.lastReqZoom = this.zoom;
      } 
...
/*		var me = new Object();
		...
		for(var i=0; i<reqTiles.length; i++) {
			var req =this.url+"?z="+reqTiles[i].z+"&x="+reqTiles[i].x+"&y="+reqTiles[i].y+"&callback=locationFeaturesHandle";
...
*/		
	...
	boundsToTiles: function(bounds){
		var tiles = [];
		var reqZoom = this.zoom;
    if(this.zoomCache) {
      if(this.zoomCache.hasOwnProperty(this.zoom+'')) {
        reqZoom = this.zoomCache[this.zoom + ''];
      }
    }
    var resolution = this.layer.map.getResolutionForZoom(reqZoom); 
		var left = Math.floor((bounds.left - this.mapBounds.left) / (resolution * this.tileWidth ));
		var bottom = Math.floor((this.mapBounds.top - bounds.bottom) / (resolution * this.tileHeight));
		var right = Math.floor((bounds.right - this.mapBounds.left) / (resolution * this.tileWidth )); 
		var top = Math.floor((this.mapBounds.top - bounds.top) / (resolution * this.tileHeight ));
