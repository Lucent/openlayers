...
OpenLayers.Strategy.TFS = OpenLayers.Class(OpenLayers.Strategy, {
    ...
    initialize: function(options) {
		...
		this.tileWidth = options.tileSize ? options.tileSize.w : 256;
		this.tileHeight = options.tileSize ? options.tileSize.h : 256;
		this.mapBounds = options.mapBounds ? options.mapBounds : new OpenLayers.Bounds(-20037508.34,-20037508.34,20037508.34,20037508.34);
	...
	//	console.log("Strategy_TFS.update");
        ...
        if (mapBounds !== null && this.invalidBounds(mapBounds)) {
			...
			this.zoom = this.layer.map.getZoom() - Math.log(this.layer.tileSize.w / map.tileSize.w) / Math.log(2);
            this.resolution = this.layer.map.maxExtent.getWidth() / this.layer.tileSize.w / Math.pow(2, this.zoom);
			if (options){
...
*/		
	...
	boundsToTiles: function(bounds){
		...
		var left = Math.floor((bounds.left - this.mapBounds.left) / (res * this.tileWidth));
		var right = Math.floor((bounds.right - this.mapBounds.left) / (res * this.tileWidth));
		var bottom = Math.floor((this.mapBounds.top - bounds.bottom) / (res * this.tileHeight));
		var top = Math.floor((this.mapBounds.top - bounds.top) / (res * this.tileHeight));
